...
<html lang="en">
  ...
  <section id="container" class="">
      ...
      <section id="main-content">
          <section class="wrapper">
                    ...
                    <div class="col-lg-12">
                        <section class="panel">
                            <header class="panel-heading">
                                <h3><i class="icon-truck"></i> POY Sales Entry</h3>
                            </header>
                        </section>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-lg-12">
                      <form class="cmxform form-horizontal tasi-form" role="form" id="commentForm" method="post" action="<?=base_url();?>poy/sales_entry_save">
                        <section class="panel">
                            <div class="panel-body">
                                ...
                                <div class="form-group col-lg-12">
                                   ...
                                   <span class="label label-danger" style="padding: 0 1em;font-size:24px;"><?=$invoice_no; ?></span>
                                </div>
                                <div class="form-group col-lg-3">
                                   <label for="sales_to">To</label>
                                   <input type="text" class="form-control" id="sales_to" name="sales_to" required>
                                </div>
                                ...
                                <div class="form-group col-lg-2">
                                   <label for="poy_lot">POY Lot No</label>
                                   <select class="select2 form-control" id="poy_lot" name="poy_lot">
                                    <option value="">Select</option>
                                    <?php
                                    foreach ($poy_lots as $row) {
                                      ?>
                                      <option value="<?=$row['poy_lot_id']; ?>"><?=$row['poy_lot_name']; ?></option>
                                      <?php
                                    }
                                    ?>
                                    </select>
                      ...
                      ?>                                     <section class="panel">
                        ...
                        <table class="table table-striped border-top" id="sample_1">
                          ...
                          <tbody>
                            <?php
                            $sno = 1;
                            if ((is_array($sales_entry) || is_object($sales_entry))&& !empty($sales_entry))
                            {
                            foreach ($sales_entry as $row) {
                              $group_row = 1;
                              $poyitems = $this->ak->get_poy_sales_items($row['sales_id']);
                              ?>
                              <?php
                              foreach ($poyitems as $item) {
                                ?>
                                <tr>
                                  <td><?=$sno; ?></td>
                                  ...
                                  <td><?=$item['denier_name']; ?>/<?=$item['poy_denier']; ?></td>
                                  <td><?=$item['poy_lot_no']; ?>/<?=$item['poy_lot']; ?></td>
                                  <td><?=$item['qty']; ?></td>
                                </tr>
                                <?php
                              }
  ...
  <script>
      ...
      $('#po_cartitems').load('<?=base_url();?>poy/poy_sales_cartitems');
      ...
      $(function(){
        $("#addtocart").click(function () {
            ...
            var url = "<?=base_url()?>poy/poy_sales_addtocart";
            var postData = 'item_id='+item_id+'&poy_lot='+poy_lot+"&poy_denier="+poy_denier+"&po_qty="+po_qty+"&item_uom="+item_uom;
            $.ajax({
                type: "POST",
                url: url,
                data: postData,
      ...
      $(function(){
        $( "a.removetocart" ).live( "click", function() {
            ...
            var url = "<?=base_url()?>poy/poy_sales_removetocart/"+row_id;
