...
<html lang="en">
   ...
   <body>
      <?php
         ...
         $machine = $this->m_masters->getactivemaster('bud_machines', 'machine_status'); 
      ?>
      <section id="container" class="">
         ...
         <section id="main-content">
            <section class="wrapper">
               ...
               <form class="cmxform form-horizontal tasi-form screen-only" role="form" id="commentForm" method="post" action="<?=base_url()?>Mir_reports/dyeing_prod_rep_1">
                  <div class="row">
                     <div class="col-lg-12">
                        <section class="panel">                            
                           <header class="panel-heading">
                           ...
                           <div class="panel-body">
                              ...
                              <div class="form-group col-lg-3">
                                 ...
                                 <select class="shade form-control select2" id="shadeName" name="shade_no">
                                    <option value="0">Select</option>
                                    <?php
                                    foreach ($shades as $row) {
                                      ?>
                                      <option value="<?=$row['shade_id']; ?>" <?=($row['shade_id'] == $shade_no)?'selected="selected"':''; ?>><?=$row['shade_name']; ?></option>
                                      <?php
                                    }
                                    ?>
                                 </select>
                              ...
                              <div class="form-group col-lg-3">
                                 <label for="item_name">Shade Code</label>
                                 <select class="shade form-control select2" id="shadeCode" name="shade_no">
                                    <option value="0">Select</option>
                                    <?php
                                    foreach ($shades as $row) {
                                      ?>
                                      <option value="<?=$row['shade_id']; ?>" <?=($row['shade_id'] == $shade_no)?'selected="selected"':''; ?>><?=$row['shade_code']; ?></option>
                                      <?php
                                    }
                                    ?>
                                 </select>
                              ...
                              <div class="form-group col-lg-3">
                                 <label for="item_name">Machine Prefix</label>
                                 <select class="form-control select2" id="machinePrefix" name="machine_prefix">
                                    <option value="0">Select</option>
                                    <?php
                                    foreach ($machine as $row) {
                                      ?>
                                      <option value="<?=$row['machine_id']; ?>" <?=($row['machine_id'] == $machine_prefix)?'selected="selected"':''; ?>><?=$row['machine_prefix']; ?></option>
                                      <?php
                                    }
                                    ?>
                                 </select>
               ...
               <div class="col-lg-12">
                  ...
                  <section class="panel">
                    ...
                    <div class="panel-body">
                           ...
                           <table id="example" class="datatable table table-bordered table-striped table-condensed cf packing-register">
                              <thead>
                                <!--<tr>
                                    ...
                                    <th color='red'><strong><?=$grand_box_num.' Boxes'; ?></strong></th>
                                    <th></th>
                                    <th color='red'><strong><?=$grand_box_net_qty.' kgs'; ?></strong></th>
                                    <th color='red'><strong><?=$grand_exess_qty.' kgs'; ?></strong></th>
                                    <th color='red'><strong><?=$grand_bal_qty.' kgs'; ?></strong></th>                                 
                                 </tr>-->
                                 <tr>
                                    <th></th>
                                    <th></th>
                                    ...
                                    <th color='red'><strong><?=array_sum($rows['box_num']).' Boxes'; ?></strong></th>
                                    <th></th>
                                    <th color='red'><strong><?=array_sum($rows['box_net_weight']).' kgs'; ?></strong></th>
                                    <th color='red'><strong><?=array_sum($rows['exess_qty']).' kgs'; ?></strong></th>
                                    <th color='red'><strong><?=array_sum($rows['bal_qty']).' kgs'; ?></strong></th>
                                 </tr>
                                 <tr>
                                    <th>#</th> 
      						            <th>Lot No</th>
                                    ...
                                    <th># of boxes</th>
                                    <th>Box Num</th>
                                    <th>Box Net Weight</th>
                                    <th>Exceed Lot Qty</th>
                                    <th>Bal. in Lot</th>
                                 </tr>
                              </thead>
      							   <tfoot >
                                 ...
                                 foreach ($rows['values'] as $row) {
                                    /**/
                                       ...
                                       <tr>
                                          ...
                                          <td><?=$row['item_name'].'/'.$row['lot_item_id']; ?></td>
                                          <td><?=$row['shade_code']; ?></td>
                                          <td><?=$row['lot_qty']; ?></td>
                                          <td><?=$rows['box_num'][$row['lot_id']];?></td>
                                          <td><?=$rows['boxes'][$row['lot_id']];?></td>
                                          <td><?=$rows['box_net_weight'][$row['lot_id']];?></td>
                                          <td><?=$rows['exess_qty'][$row['lot_id']];?></td>
                                          <td><?=$rows['bal_qty'][$row['lot_id']];?></td>
                                       </tr>
                        ...
                        {
                           ?>
                           <h3 align='center' ><strong>No Data Found</strong></h3>
                           <?php
                        }
                        ?>
                           <hr>
      ...
      $(".shade").change(function(){
        $("#shadeName").select2("val", $(this).val());
        $("#shadeCode").select2("val", $(this).val());       
        return false;
      });
      </script>
   </body>
