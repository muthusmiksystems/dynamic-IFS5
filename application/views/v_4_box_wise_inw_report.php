               ...
               <div class="row screen-only">
                  <section class="panel">
                     <header class="panel-heading">
                        ...
                        <button class="btn btn-info" align='to'><a href="<?=base_url(); ?>Mir_reports/tot_<?=$form;?>_item_wise_4/<?=$term; ?>/<?=$year;?>/<?=$o_month;?>" >Back</a></button></h3>
                     </header>
                  </section>
               </div>
               <div class="row screen-only">
                    ...
                    <div class="panel-body">
                        ...
                        if($box_detail)
                        {
                           ...
                           foreach ($box_detail as $box) {
                              ...
                              $tot_box++;
                              if($form=='inw'){
                                 $stock[$box['box_id']]=$box['nt_weight']-$this->m_masters->get_tot_field_value('bud_sh_predel_items','delivery_qty',array('box_id'=>$box['box_id']));
                              }
                              elseif ($form=='otw') {
                                 $cash_qty=$this->m_masters->get_tot_field_value('bud_sh_cash_invoice_items','delivery_qty',array('box_id'=>$box['box_id']));
                                 $credit_qty=$this->m_masters->get_tot_field_value('bud_sh_credit_invoice_items','delivery_qty',array('box_id'=>$box['box_id']));
                                 $est_qty=$this->m_masters->get_tot_field_value('bud_sh_quotation_items','delivery_qty',array('box_id'=>$box['box_id']));
                                 $stock[$box['box_id']]=$cash_qty+$credit_qty+$est_qty;
                              }
                           }
                        }
                        ?>
                           <table id="example" class="datatable table table-bordered table-striped table-condensed cf packing-register">
                              <thead>
                                <tr>
                                    ...
                                    <th><strong class='text-danger'><?=array_sum($stock).' Kgs'; ?></strong></th>
                                 </tr>
                                 <tr>
                                    <th >#</th>  
                                    <th >Box No</th>
                                    ...
                                    <th >Gr wt</th>
                                    <th >Net Wt</th>
                                    <th ><?=($form=='inw')?'Stock Qty':'Delivered Qty'?></th>
                                 </tr>
                              </thead>
                              <tfoot>
                                 <tr>
                              ...
                              <?php
                                 ...
                                 foreach ($box_detail as  $box){
                                    ...
                                    <tr>
                                       ...
                                       <td><?=$box['lot_no'];?></td>
                                       <td><?=($box['supplier_id'])?:'UNIT 1';?></td>
                                       <td><?=$box['prepared_by'];?></td>
                                       <td><?=date('d-M-y H:i',strtotime($box['packed_on']));?></td>
                                       <td><?=$box['stock_room_name'];?></td>
                                       <td><?=$box['no_cones'];?></td>
                                       <td><?=$box['gr_weight'];?></td>
                                       <td><?=$box['nt_weight'];?></td>
                                       <td><?=$stock[$box['box_id']]?></td>
                                    </tr>
                                    <?php
                                    $sno++;
                                 }  
