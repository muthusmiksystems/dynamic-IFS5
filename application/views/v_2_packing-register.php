      ...
      <section id="container" class="">
         ...
         <section id="main-content">
            <section class="wrapper">
               ...
               <?php
               $items = $this->m_masters->getactivemaster('bud_te_items', 'item_status');                      $item_groups= $this->m_masters->getactivemaster('bud_te_itemgroups', 'group_status');
               $uoms = $this->m_masters->getactivemaster('bud_uoms', 'uom_status');
               $item_name = null;                      ?>
               <form class="cmxform form-horizontal tasi-form screen-only" role="form" id="commentForm" method="post" action="<?=base_url()?>reports/packingRegister">
                  <div class="row">
                     <div class="col-lg-12">
                        <section class="panel">                                               <header class="panel-heading">
                           ...
                           <div class="panel-body">
                              ...
                              </div>                                        <!--to include itemgroup filter in packing register-->
                              <div class="form-group col-lg-3">
                    ...
                    <div class="panel-body">
                        ...
                        ?>                                    <table id="example" class="table datatable table-bordered table-striped table-condensed cf packing-register">
                         <thead>
                           ...
                           <tr>
                              <th>#</th>                                               <th>Box No</th>
                              <th>Item group</th><!--to include itemgroup filter in packing register-->
                              ...
                              <th>Nt. Weight</th>
                              <th>Total Meters</th>
                              <th class="screen_only">Status</th>
                           </tr>
                           </thead>
                           <tbody>
                           <?php
                           ...
                           foreach ($outerboxes as $row) {
                              ...
                              <tr>
                                 <td><?=$sno; ?></td>
                                 <td><?=$box_no; ?></td>
                                 <td><?=$row['group_name'];?></td><!--to include itemgroup filter in packing register-->
                                 <td><?=$row['item_name'].'/'.$packing_innerbox_items;?></td>
                                 ...
                                 <td class="screen_only">
                                    ...
                                    <?php
                                    if(($row['predelivery_status']==1)&&($row['delivery_status']==1))
                                    {
                                    ?>
                                       <button class="btn btn-success btn-xs">Stock</button></br>
                                    <?php
                                    }
                                    elseif (($row['predelivery_status']==0)&&($row['delivery_status']==1)) {
                                    ?>
                                       <button class="btn btn-warning btn-xs">Reservered</button></br>
                                    <?php
                                    }else if(($row['predelivery_status']==0)&&($row['delivery_status']==0)) {
                                       $inv_no=$this->m_reports->get_box_invoiced_te($box_no);
                                    ?>
