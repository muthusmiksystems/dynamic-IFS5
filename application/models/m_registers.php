...
class M_registers extends CI_Model
{
    ...
    function convert_to_string($dates)
    {
        return implode(" || ", array_map(function ($v) {
            return sprintf("FIND_IN_SET('%s', date(rate_changed_on)) != 0", $v);
        }, array_values($dates)));
    }
    ...
    function get_box_status_yt($box_prefix = null, $box_no, $from_box_no = null, $to_box_no = null) //ER-09-18#-63
    {
        $this->db->select('bud_yt_packing_boxes.*,bud_items.item_name,bud_shades.shade_code,bud_shades.shade_name');
        if ($box_no) {
        ...
        $this->db->join('bud_items', 'bud_yt_packing_boxes.item_id=bud_items.item_id');
        $this->db->join('bud_shades', 'bud_yt_packing_boxes.shade_no=bud_shades.shade_id');
        return $this->db->get('bud_yt_packing_boxes')->result();
    }
    function get_status_details_yt($box_id = null) //ER-09-18#-63
    {
        $this->db->select('*');
        if ($box_id) {
            $this->db->where('box_id', $box_id);
        }
        $p_del_items = $this->db->get('dyn_yt_predelivery_items')->result_array();
        $data['pdc_no'] = '';
        $data['dc_no'] = '';
        $data['inv_no'] = '';
        foreach ($p_del_items as $p_del_item) {
            $data['pdc_no'] = ($p_del_item['p_delivery_id']) ? 'PDC/' . $this->m_masters->getmasterIDvalue('bud_yt_predelivery', 'p_delivery_id', $p_del_item['p_delivery_id'], 'p_delivery_id') : '';
            $data['dc_no'] = ($p_del_item['p_delivery_id']) ? $this->m_masters->getmasterIDvalue('bud_yt_delivery', 'delivery_id', $p_del_item['delivery_id'], 'dc_no') : '';
            $data['inv_no'] = ($p_del_item['invoice_id']) ? $this->m_masters->getmasterIDvalue('bud_yt_invoices', 'invoice_id', $p_del_item['invoice_id'], 'invoice_no') : '';
        }
        return $data;
    }
}
