...
class Delivery extends CI_Controller
{
	...
	function delivery_1_delete() //ER-09-18#-58
	{
		...
		$remarks = $this->input->post("remarks");
		if ($delivery_id) {
			$result = $this->m_masters->update_delete_status_delivery_yt($delivery_id, $remarks);
		}
	...
	function delivery_1()
	{
		...
		$data['page_title'] = 'Pdc transfer to DC';
		$data['customers'] = $this->m_masters->getactivemaster('bud_customers', 'cust_status');
	...
	function gray_yarn_soft_delivery($delivery_id = false)
	{
		...
		if ($this->input->post('save_as_del')) {
			...
			if (is_array($data['outer_boxes'])) {
				foreach ($data['outer_boxes'] as $box) {
					$this->db->query("UPDATE bud_yt_packing_boxes SET predelivery_status = '0', delivery_status = '0' WHERE box_id = '" . $box . "'");
				}
			}
		}
