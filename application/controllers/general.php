...
class General extends CI_Controller
{
	...
	function getGeneralStock()
	{
		...
		$transfered_qty = $this->m_general->get_transfered_qty($stockroom_id);
		$received_qty = $this->m_general->get_received_qty($stockroom_id);
		foreach ($result as $row) {
			$qty = $row['qty'];
			$type = $row['type'];
			if ($type == '+') {
	...
	function IOweYouSave()
	{
		...
		if ($iou_voucher_id) {
			...
			redirect(base_url('general/ioweu_receipt/' . $iou_voucher_id));
		} else {
			$this->session->set_flashdata('error', 'That action is not valid, please try again');
			redirect(base_url() . "general/IOweYou", 'refresh');
		}
	...
	public function ioweu_receipt($iou_voucher_no = '')
	{
		$data['activeTab'] = 'general';
		$data['activeItem'] = 'IOweYou';
		$data['page_title'] = 'IOweYou Receipt';
		$voucher = $this->m_general->get_iowe_you_voucher($iou_voucher_no);
		$data['voucher'] = $voucher;
		$data['voucher_items'] = $this->m_general->get_ioweu_items($iou_voucher_no);
		$this->load->view('ioweu-receipt', $data);
	}
	function IOweYouReturn()
	{
		$data['activeTab'] = 'general';
		$data['activeItem'] = 'IOweYouReturn';
		$data['page_title'] = 'I Owe You Return Form';

		$data['vocher'] = $this->ak->get_vocher_no();
		...
		$data['js_common'] = array('js/common-scripts.js');
		$data['js_thispage'] = array('js/form-validation-script.js', 'js/form-component.js', 'js/dynamic-table.js');
		$this->load->view('v_general-iowe-you-return.php', $data);
	}
	function get_vocher_detail()
	{
		$sum_q = 0;
		$sum_s = 0;

		$vocher_id = $_POST['vocher'];
		$tab = $this->ak->get_vocher_detail($vocher_id);
		echo "
<h3>Vocher No : " . $vocher_id . "</h3>
<table class='table table-striped border-top table-bordered' id='sample_'>
                          <thead>
                            <tr>
                                <th>#</th>
                                <th>Item Name</th>
		...
		for ($a = 0; $a < count($tab); $a++) {
			if ($tab[$a]['cleared'] == 0) {
				$b = $a + 1;
				$sum_q += $tab[$a]['qty_required'];
				$sum_s += $tab[$a]['amt_required'];
				echo "
			 <tr>
                <td>" . $b . "</td>
                <td>" . $tab[$a]['item_name'] . "</td>
                <td>" . $tab[$a]['qty_required'] . "</td>
				<td>" . $tab[$a]['amt_required'] . "</td>
				<td><input type='text' id='spend" . $tab[$a]['id'] . "'></td>
				<td><input type='text' id='balance" . $tab[$a]['id'] . "'></td>
				<td><input type=button class='btn btn-danger' onclick=close_vocher(" . $tab[$a]['id'] . "," . $tab[$a]['iou_voucher_id'] . ") value=Clear></td>
             </tr>
		";
			}
		}

		echo "<tr><td></td><td></td><td>Total Qnty : " . $sum_q . "</td><td>Total amount : " . $sum_s . "</td><td></td><td></td><td>
			<!-- <button class='btn btn-primary' onclick=close_vocher_all('" . $vocher_id . "')>Clear All</button> -->
		</td></tr>";
		echo "</tbody></table>";
	}
	function mail()
	{
		...
		$this->email->from($from . '.dynamic@gmail.com', $from);
		$this->email->to($to);
		$this->email->subject('I owe you vocher form');
		$this->email->message($msg);
		$this->email->send();
		echo $this->email->print_debugger();
		echo $msg;
	}
	function complete_vocher_item()
	{
		$this->ak->complete_vocher_item($_POST['id'], $_POST['spend'], $_POST['balance']);
	}
	/*function complete_vocher()
	{
		$this->ak->complete_vocher($_POST['id']);
		redirect(base_url().'general/IOweYou', 'refresh');
	}*/
}
