...
class Register_Model extends CI_Model {
    ...
    function get_box_dc_details($box_id='')//ER-09-18#-61
    {
        ...
        foreach ($inv['cash'] as $cash) {
           $data['cash'][]=$cash['invoice_no'].'-'.$cash['item_rate'].'-'.$cash['delivery_qty'].'-'.$cash['item_rate']*$cash['delivery_qty'];//ER-09-18#-65
           $data['dc_qty'][]=$cash['delivery_qty'];
           $data['cash_price'][]=$cash['item_rate']*$cash['delivery_qty'];
        }
        foreach ($inv['credit'] as $credit) {
           $data['credit'][]=$credit['invoice_no'].'-'.$credit['item_rate'].'-'.$credit['delivery_qty'].'-'.$credit['item_rate']*$credit['delivery_qty'];//ER-09-18#-65
        ...
        foreach ($inv['quote'] as $quotation) {
           ...
           $data['quotation'][]=$quotation['quotation_no'].'-'.$quotation['item_rate'].'-'.$quotation['delivery_qty'].'-'.$quotation['item_rate']*$quotation['delivery_qty'];//ER-09-18#-65
        ...
        foreach ($dcs as $dc) {
            ...
            $data['dc'][]='SHOP-DC-'.$dc['delivery_id'].'-'.$item_rate.'-'.$dc['delivery_qty'].'-'.$dc['delivery_qty']*$item_rate;//ER-09-18#-65
            $data['dc_price'][]=$item_rate*$dc['delivery_qty'];
        }
        return $data;
    }
    //ER-10-18#-66
    public function get_delivery_boxes_reg($filter)//ER-10-18#-66
    {
        ...
        if($filter['dc_id'])
        {
            $this->db->where('bud_sh_delivery_items.delivery_id', $filter['dc_id']);
        }
        if($filter['box_id'])
        {
            $this->db->where('bud_sh_delivery_items.box_id', $filter['box_id']);
        }
        if($filter['shade_code'])
        {
            $this->db->where('bud_sh_delivery_items.shade_id', $filter['shade_code']);
        }
        if($filter['item_id'])
        {
            $this->db->where('bud_sh_delivery_items.item_id', $filter['item_id']);
        }
        if($filter['dc_status']=='1')
        {
            $this->db->where('bud_sh_delivery.delivery_status',0);
        }
        if($filter['dc_status']=='2')
        {
            $this->db->where('bud_sh_delivery.delivery_status',1);
        }
        $this->db->where('bud_sh_delivery_items.delivery_is_deleted',1);
