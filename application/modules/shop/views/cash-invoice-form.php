            ...
            <div class="col-lg-12">
                <section class="panel">
                    ...
                    <div class="panel-body">
                        ...
                        <form class="cmxform" role="form" method="post" id="ajaxForm" action="<?php echo base_url('shop/sales/cash_invoice_save/' . $p_delivery_id); ?>">
                            <?php if (count($cash_inv_terms) > 0) : ?>
                                <?php
                                $concern = $this->Sales_model->get_concern($cash_inv_terms['p_concern_id']);
                                $customer = $this->Sales_model->get_customer($cash_inv_terms['p_customer_id']);
                                if ($concern) {
                                    $invoice_count = $this->Sales_model->get_invoice_count($concern->concern_id, 'cash'); //ER-07-18#-22
                                    ...
                                    $cash_inv_no = $prefix . '-' . $financialyear . '-' . ($bill_year + 1) . '/' . ($invoice_count + 1 + $invoice_start);
                                }
                                ?>
                                <input type="hidden" name="p_concern_id" value="<?php echo $cash_inv_terms['p_concern_id']; ?>">
                                <input type="hidden" name="p_customer_id" value="<?php echo $cash_inv_terms['p_customer_id']; ?>">
                                <input type="hidden" name="p_delivery_to" value="<?php echo $cash_inv_terms['p_delivery_to']; ?>">
                                <input type="hidden" name="name" value="<?php echo $cash_inv_terms['name']; ?>">
                                <input type="hidden" name="mobile_no" value="<?php echo $cash_inv_terms['mobile_no']; ?>">
                                <input type="hidden" name="remarks" value="<?php echo $cash_inv_terms['remarks']; ?>">
                                <input type="hidden" name="cash_inv_no" value="<?php echo $cash_inv_no; ?>">
                            <?php endif; ?>
