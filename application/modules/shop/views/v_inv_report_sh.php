<?php include APPPATH.'views/html/header.php'; ?>
	...
	<section id="main-content">
		<section class="wrapper">
            <form class="cmxform form-horizontal tasi-form screen-only" role="form" id="commentForm" method="post" action="<?=base_url()?>shop/registers/inv_report_sh">
              <div class="row">
                 <div class="col-lg-12">
                    <section class="panel">                            
                       ...
                       <div class="panel-body">
                          ...
                          <div class="form-group col-lg-3">
                             <label for="credit_cash">Credit/cash</label>
                             <select class=" form-control" id="credit_cash" name="status">
                                <option value="all">Select</option>
                                <option value="cash" <?=($status=='cash')?'selected="selected"':''; ?>>Cash</option>
                                <option value="credit" <?=($status=='credit')?'selected="selected"':''; ?>>Credit</option>
                             </select>
                          </div>
                          <div class="form-group col-lg-3">
                          ...
                          <div class="col-sm-4 text-right">
                            ...
                            <table class="table table-bordered dataTables">
                                ...
                                <tbody>
                                    ...
                                    $sno = 1;
                                    ?>
                                    <?php foreach($invoices_list as $key=>$invoices): ?>
                                    <?php if(sizeof($invoices) > 0): ?>
                                        <?php foreach($invoices as $row): ?>
                                            ...
                                            $check=1;//ER-08-18#-29
                                            if($key=='credit'){
                                              $invoices_items =$this->Sales_model->get_credit_inv_items($row->invoice_id,'');//ER-08-18#-29
                                            }
                                            elseif ($key=='cash') {
                                              $invoices_items =$this->Sales_model->get_cash_inv_items($row->invoice_id,'');//ER-08-18#-29
                                            }
                                            ...
                                            <tr>
                                                ...
                                                <td><?php if($key=='credit'): ?>
                                                    <a target="_blank" href="<?php echo base_url('shop/sales/print_credit_invoice/'.$row->invoice_id); ?>" class="btn btn-xs btn-primary">Print</a>
                                                    <?php endif; ?>
                                                    <?php if($key=='cash'): ?>
                                                    <a target="_blank" href="<?php echo base_url('shop/sales/print_cashinvoice/'.$row->invoice_id); ?>" class="btn btn-xs btn-primary">Print</a>
                                                    <?php endif; ?>
                                                </td><!--//ER-09-18#-61-->
                                            </tr>
                                        <?php endforeach; ?>
